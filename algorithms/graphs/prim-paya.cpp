...
{
	std::priority_queue<Label<T>, std::vector<Label<T>>, std::greater<Label<T>>> Q;
	std::set<int> visited;
	...
	while (!Q.empty())
	{
		...
		if (visited.find(current_vertex.vertex_ID) == visited.end())
		{
			...
			// create a label for the destination vertex and add it to the Q(heap)
			for (auto e : G.outgoing_edges(current_vertex.vertex_ID))
			{
				...
				{
					Q.emplace(neighbor_vertex_ID, new_distance_to_frontier);
...
//https://compiler-explorer.com/z/nbvrqzM91
